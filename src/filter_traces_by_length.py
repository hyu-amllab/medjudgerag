#!/usr/bin/env python3
"""
filter_traces_by_length.py — Filter samples exceeding max sequence length.

Tokenizes each sample using the EXACT same prompt and chat template
as the training script (train_medjudgerag_mistral.py) to ensure
the token count matches what the model actually sees during training.

Usage:
    python filter_traces_by_length.py \
        --input data/full_teacher_traces_postprocessed.jsonl \
...
def build_full_training_text(item, tokenizer):
    ...
    # Reconstruct assistant response: KG + <ANALYSIS> + answer_target
    kg = item.get("kg", "")
    answer_target = item.get("answer_target", "")
    if kg.strip():
        assistant_content = kg + ANALYSIS_DELIMITER + answer_target
    else:
        assistant_content = "Entities:\n\nRelations:" + ANALYSIS_DELIMITER + answer_target
