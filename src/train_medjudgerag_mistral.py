#!/usr/bin/env python3
"""
train_medjudgerag_mistral.py - MedJudgeRAG LoRA SFT (KG → Analysis → Answer).

Single model generates KG, then analyzes options, then answers.
Uses Mistral-7B-Instruct-v0.3 as the default base model.

Weighted loss masking:
    - System/User prompt: loss weight 0 (masked via labels=-100)
    - KG block (assistant, before <ANALYSIS>): loss weight = --kg_weight (default 0.0)
    - <ANALYSIS> onwards: loss weight = --analysis_weight (default 1.0)

Usage:
    # Multi-GPU with DeepSpeed ZeRO-2:
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \\
    CUDA_VISIBLE_DEVICES=2,3 accelerate launch --num_processes 2 --mixed_precision bf16 \\
        src/train_medjudgerag_mistral.py --deepspeed configs/deepspeed_zero2.json --epochs 3 --max_seq_len 8192

    # Single GPU:
    CUDA_VISIBLE_DEVICES=0 python src/train_medjudgerag_mistral.py --epochs 3 --max_seq_len 8192
"""
...
def reconstruct_analysis_text(answer_target):
    """Reconstruct semi-structured analysis text from answer_target.

    answer_target can be:
    - str: raw GPT output text (preferred, used as-is)
    - dict: parsed format from validate step (reconstruct text)
    """
    if isinstance(answer_target, str):
        return answer_target.strip()

    if not isinstance(answer_target, dict):
        return ""

    # Reconstruct from parsed dict (matches GPT-5.1 target format:
    # Doc → KG → Evidence → Verdict per option, Answer_choice at end)
    ...
    for opt in ["A", "B", "C", "D"]:
        ...
        kg_refs = entry.get("kg_refs", [])

        lines.append(f"[{opt}] Doc: {json.dumps(doc_ids)}")
        if kg_refs:
            lines.append(f"KG: [{', '.join(kg_refs)}]")
        else:
            lines.append("KG: []")
...
def build_chat_messages(sample):
    """Build chat messages for end-to-end training.

    Assistant content = KG block + <ANALYSIS> + analysis text.
    """
    ...
    # KG block (from M_KG prediction)
    kg = sample.get("kg", "").strip()
    if not kg:
        kg = "Entities:\n\nRelations:"

    # Analysis text (from GPT-5.1 answer target)
    analysis_text = reconstruct_analysis_text(sample.get("answer_target", ""))

    # Combine: KG + delimiter + analysis
    assistant_content = kg + ANALYSIS_DELIMITER + analysis_text
...
def load_data(data_path, split_filter=None):
    """Load JSONL data for MedJudgeRAG SFT.
...
def find_boundaries(ids, assistant_header_ids, analysis_pattern_ids_list):
    ...
    """
    analysis_pos, _, assistant_pos = find_analysis_delimiter(
        ids, assistant_header_ids, analysis_pattern_ids_list
    )

    if assistant_pos == -1 or analysis_pos == -1:
        return -1, -1

    prompt_end = assistant_pos + len(assistant_header_ids)
    return prompt_end, analysis_pos
...
class WeightedLossCollator:
    ...
    Loss regions:
        ...
        [assistant: KG block]   → loss_weights=kg_weight
        [<ANALYSIS> + analysis] → loss_weights=analysis_weight
...
def main():
    parser = argparse.ArgumentParser(
        description="MedJudgeRAG LoRA SFT (KG → Analysis → Answer)"
    )
    parser.add_argument("--train_data", default="data/full_teacher_traces_mistral_8192.jsonl")
    parser.add_argument("--val_data", default="data/full_teacher_traces_mistral_8192.jsonl")
    ...
    parser.add_argument("--output_dir", default="outputs/medjudgerag_mistral_lora")
    ...
    parser.add_argument("--kg_weight", type=float, default=0.0,
                        help="Loss weight for KG region (default: 0.0)")
    ...
    # Encode <ANALYSIS> delimiter variants
    analysis_pattern_ids_list = []
    seen_patterns = set()
    print("<ANALYSIS> delimiter token IDs:")
    for pattern in ANALYSIS_SEARCH_PATTERNS:
        pattern_ids = tokenizer.encode(pattern, add_special_tokens=False)
        key = tuple(pattern_ids)
        if key not in seen_patterns:
            seen_patterns.add(key)
            analysis_pattern_ids_list.append(pattern_ids)
            print(f"  {pattern!r} -> {pattern_ids} (len={len(pattern_ids)})")

    # Context-aware detection: some tokenizers encode <ANALYSIS> differently
    # when embedded in text vs standalone. Extract actual token patterns from
    # the first few training samples to capture context-dependent variants.
    print("Probing context-aware <ANALYSIS> patterns from training samples...")
    for probe_sample in train_samples[:5]:
        messages = build_chat_messages(probe_sample)
        probe_text = tokenizer.apply_chat_template(
            messages, tokenize=False, add_generation_prompt=False
        )
        probe_ids = tokenizer(probe_text, add_special_tokens=False)["input_ids"]
        # Decode token-by-token to find <ANALYSIS> boundary
        for i in range(len(probe_ids)):
            # Check windows of length 5-15 starting at i
            for wlen in range(5, min(16, len(probe_ids) - i + 1)):
                chunk = tokenizer.decode(probe_ids[i:i + wlen])
                if "<ANALYSIS>" in chunk and "<ANALYSIS>" not in tokenizer.decode(probe_ids[i + 1:i + wlen]):
                    candidate = probe_ids[i:i + wlen]
                    # Trim trailing tokens after <ANALYSIS>
                    for end in range(len(candidate), 0, -1):
                        decoded = tokenizer.decode(candidate[:end])
                        if "<ANALYSIS>" in decoded:
                            candidate = candidate[:end]
                            break
                    key = tuple(candidate)
                    if key not in seen_patterns:
                        seen_patterns.add(key)
                        analysis_pattern_ids_list.append(list(candidate))
                        print(f"  (context) {tokenizer.decode(candidate)!r} -> {list(candidate)} (len={len(candidate)})")
                    break
            else:
                continue
            break
